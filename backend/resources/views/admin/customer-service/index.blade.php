...
@section('page_title', 'Customer Service')
...
$(document).ready(function() {
    ...
    function loadMessages(userId, silent = false) {
        $.ajax({
            url: '{{ url("admin/customer-service/messages") }}/' + userId,
            type: 'GET',
            dataType: 'json',
    ...
    $('#chatForm').off('submit').on('submit', (function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.customer-service.send") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    setInterval(function() {
        $.ajax({
            url: '{{ route("admin.customer-service.unread-count") }}',
            type: 'GET',
            success: function(response) {
                if (response.count > 0) {
